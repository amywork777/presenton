...
function UploadPanel({
  ...
  return (
    <main className="flex min-h-screen flex-col items-center bg-white font-syne">
      ...
      <section className="mt-12 w-full max-w-[556px] px-4">
        <div className="relative ml-6 w-max rounded-t-[22px] border border-b-0 border-[#EDEEF4] bg-white px-3 pb-3 pt-2 shadow-[0_0_16px_rgba(80,71,230,0.08)]">
          <button
            type="button"
            onClick={() => inputRef.current?.click()}
            className="flex h-[32px] items-center gap-1.5 rounded-[80px] bg-white px-3 text-[11px] font-semibold text-black shadow-[0_0_4px_rgba(0,0,0,0.06)]"
          >
        ...
        <div className="rounded-[28px] border border-[#EDEEF4] bg-white p-2 shadow-[0_0_16px_rgba(80,71,230,0.08)]">
          <div className="rounded-[18px] border border-[#EDEEEF] bg-[#FEFEFF] p-2">
            <div
              className={`relative h-[110px] overflow-hidden rounded-[12px] border border-[#E8E8EF] bg-white ${
            ...
            >
              ...
              {selectedFile ? (
                <>
                  <div className="absolute inset-y-0 left-0 w-[90%] bg-[#F6F6FA]" />
                  <div className="relative z-10 flex h-full items-center px-6">
                    <div className="min-w-0">
                      <p className="truncate text-[15px] font-medium text-[#20212A]">
                        {selectedFile.name}
                      </p>
                  ...
                  <button
                    ...
                    className="absolute right-4 top-1/2 z-20 flex h-8 w-8 -translate-y-1/2 items-center justify-center rounded-full bg-[#EFF0F4] text-black disabled:opacity-50"
                ...
                <div className="flex h-full flex-col items-center justify-center">
                  <img
                    ...
                    draggable={false}
                  />
                  <p className="mt-3 text-xs font-normal text-[#8A8A93]">
            ...
            <div className="mt-2 flex items-center justify-end gap-3">
              <span className="inline-flex items-center gap-1 text-[11px] font-semibold text-[#0E9F5E]">
                5 Credits
        ...
        <ul className="mx-auto mt-5 flex max-w-[360px] items-center justify-between gap-4">
          {["Test in Real Time", "Max 100MB", "5min Generation"].map((item) => (
            <li key={item} className="flex items-center gap-1.5">
              <span className="h-2 w-2 rounded-full bg-[#EBE9FE]" />
              <span className="text-[12px] font-normal text-[#3A3A3A]">{item}</span>
            </li>
          ))}
        </ul>
      </section>
...
function AnalyzePanel({
  ...
  const fontChips = fontsData ? uniqueFontChips(fontsData).slice(0, 6) : [];
    ...
    <main className="min-h-screen bg-white px-6 pb-24 pt-[132px] font-syne">
      <section className="mx-auto w-full max-w-[496px]">
        <div className="relative pl-12">
          <div className="absolute left-[15px] top-8 h-[146px] w-px bg-[#ECECF2]" />
          <div className="relative">
            ...
            <div className="mb-6 flex items-center justify-between">
              <h2 className="text-[17px] font-semibold text-black">Creating Template</h2>
              <span className="text-[10px] font-medium text-[#70737D]">Completed</span>
            </div>
            <div className="rounded-[5px] border border-[#E8E8EF] bg-[#F7F7FA] px-3 py-3">
              <div className="flex items-center justify-between">
                ...
                <span className="flex h-4 w-4 items-center justify-center rounded-full bg-[#2CA36B] text-[10px] text-white">
          ...
          <div className="relative mt-7">
            ...
            <div className="mb-3 flex items-center justify-between">
              ...
              <span className="text-[10px] font-medium text-[#70737D]">Current State</span>
            </div>
            <div className="rounded-[5px] border border-[#E8E8EF] bg-[#F7F7FA] px-3 py-3">
              <div className="mb-2 flex items-start justify-between gap-3">
                <div className="flex items-center gap-1.5 text-[13px] font-medium text-black">
                  <span className="text-[#EF5D3E]">△</span>
                  {missingFonts.length || 0} fonts require attention
                </div>
                {isUploading ? (
                  <Loader2 className="h-5 w-5 animate-spin text-[#7A5AF8]" />
                ) : (
                  <span className="h-5 w-5 rounded-full border-2 border-[#7A5AF8] border-r-transparent" />
                )}
              </div>
              <div className="flex flex-wrap gap-2">
                {fontChips.length > 0 ? (
                  fontChips.map((font, index) => {
                    const isMissing = index < missingFonts.length;
                    return (
                      <span
                        key={`${font.name}-${index}`}
                  ...
                  <span className="rounded-full border border-[#E1E2E8] bg-[#F0F1F4] px-3 py-1 text-[11px] text-[#555862]">
                    No missing fonts
                  </span>
                )}
              </div>
            ...
            <div className="mt-3 space-y-4">
              {(missingFonts.length ? missingFonts : [{ name: "Poppins", url: null }]).map(
                (font, index) => {
                    ...
                    <div key={`${fontName}-${index}`} className="space-y-2">
                      ...
                      <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-3">
                        ...
                        <button
                          ...
                          className="flex h-10 items-center gap-2 rounded-[5px] border border-[#E2E3E8] bg-white px-3 text-left text-[13px] text-[#72757F]"
                        >
                        ...
                        <div className="relative">
                          <select
                            ...
                            className="h-10 w-full appearance-none rounded-[5px] border border-[#E2E3E8] bg-white px-4 pr-8 text-[13px] text-[#282A32] outline-none"
      ...
      <button
        ...
        className="fixed bottom-9 right-7 inline-flex h-[38px] items-center justify-center rounded-[58px] px-5 text-[14px] font-medium text-black disabled:cursor-not-allowed disabled:opacity-60"
        style={{ background: pillGradient }}
      >
...
function PreviewPanel({
  ...
  const urls = previewData.slide_image_urls ?? [];
  const currentUrl = urls[selectedIndex] ?? urls[0];

  return (
    <main className="min-h-screen bg-white px-6 pb-28 pt-[103px] font-syne">
      <div className="mx-auto w-full max-w-[944px] overflow-hidden rounded-[7px] bg-white shadow-[0_4px_18px_rgba(16,24,40,0.12)]">
        {currentUrl ? (
          <img
            src={resolveBackendAssetUrl(currentUrl)}
            alt={`Slide ${selectedIndex + 1}`}
            className="block w-full"
            draggable={false}
          />
        ) : (
          <div className="aspect-video w-full bg-[#F7F7FA]" />
        )}
      </div>

      <ThumbnailStrip urls={urls} selectedIndex={selectedIndex} onSelect={onSelect} />
    </main>
  );
}
...
const CustomTemplatePage = ({
  ...
  const showPreview = state.step === "slides-preview" && state.previewData;
  const showReview =
    (state.step === "template-creation" || state.step === "completed") && slides.length > 0;
  const generatedSlidesReady =
    state.step === "completed" && slides.some((slide) => slide.processed && !slide.error);
  ...
  useEffect(() => {
    if (state.previewData?.slide_image_urls?.length) {
  ...
  const handleGenerateTemplate = useCallback(() => {
    if (!state.previewData || state.isLoading) return;
    trackEvent(MixpanelEvent.Templates_Build_Template_Clicked, {
      source: "template_studio_standard_v2",
      slide_count: state.previewData.slide_image_urls.length,
    });
    void initTemplateCreation({
      name: defaultTemplateName,
    });
  }, [defaultTemplateName, initTemplateCreation, state.isLoading, state.previewData]);
    ...
    if (showPreview) {
      return (
        <GradientPillButton onClick={handleGenerateTemplate} disabled={state.isLoading}>
          {state.isLoading ? "Generating Template" : "Generate Template"}
  ...
  return (
    <div className="relative min-h-screen bg-white">
      <div className={libreStatus === "ready" ? "" : "pointer-events-none select-none blur-[3px]"}>
        ...
        {showPreview && state.previewData ? (
          <PreviewPanel
            previewData={state.previewData}
            selectedIndex={previewSlideIndex}
            onSelect={setPreviewSlideIndex}
          />
        ) : null}
